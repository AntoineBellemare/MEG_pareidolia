...
data = pd.read_csv('../../../Merged_dataframes/df_ALL_metadata_MEG_sub00to11_epo_long_ALL_new.csv')
TO_PREDICT = 'parei'

min_obs = 50
#n_seeds = 50
savename='epo_RT_before_allSpectral_single_feature_balanced'
variables = ['delta', 'theta', 'alpha', 'low_beta', 'high_beta', 'gamma1', 'gamma2']
n_permutations = 100
...
for participant in participants[:]:
    ...
    if len(df_list) > 0:
        ...
        df_Acc.to_csv('../../../OUTPUTS/ML_accuracy_scores/classifiers_{}_{}_-p{}_{}.csv'.format(savename, variable, str(participant), str(n_permutations)), index=False)
        all_df_Acc.append(df_Acc)
df_Acc_final = pd.concat(all_df_Acc)
df_Acc_final.to_csv('../../../OUTPUTS/ML_accuracy_scores/classifiers_{}_{}_{}.csv'.format(savename, variables, str(n_permutations)), index=False)
