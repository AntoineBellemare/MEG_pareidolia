...
def epoching(subj, task, run, window = None, RT_thresh=5000):
    ...
    if task == 'pareidolia':
        ...
        if window == 'RT':
            for e in range(len(events)):  #start epochs at median RT for non-pareidolai trials
                ...
                try:
                    if events[e][2] == 6 and ((events[e][0]-events[e-1][0]) < (tmax-tmax)):
                        events[e][2] == 666  # Replace trials when epoch position does not fit in the data size.
                        print('epochs window too early')
                    if events[e][2] == 6 and ((events[e+1][0]-events[e][0]) < (tmax-tmax)):
                        events[e][2] == 666  # Replace trials when epoch position does not fit in the data size.
    ...
    #Here we call the function that generates the epochs, using all the necessary information created earlier
        ...
        if len(epochs) == 51 and window == 'before':
            ...
            if subj+run_str != '114':
                if subj+run_str != '027':
                    try:
                        ...
                        epochs_file, epochs_path = get_pareidolia_bids(FOLDERPATH, subj, task, run_str, stage = 'epo_RS_3sec')
