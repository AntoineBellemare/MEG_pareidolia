        ...
        for run in RUN_LIST[task]:
            ...
            for i in range(len(epochs_data)):
                ...
                for j in range(len(epochs_data[0])):
                    results, info = nk.complexity(
                        epochs_data[i][j], which=which_complex
                    )
                    complex_trials = complex_trials._append(results)
                    print(results)
            ...
            complex_file, complex_path = get_pareidolia_bids(
                FOLDERPATH, subj, task, run, stage="Complexity_fast_RS_3sec"
            )
