...
def compute_consonance(epochs, peaks_function='EMD', precision=0.5, tmin = None, tmax = None):
    ...
    #epochs = epochs.apply_baseline((-2.0, -1.5))
...
for subj in SUBJ_LIST:
    for task in TASK_LIST:
        for run in RUN_LIST[task]:
            try:
                epochs_name, epochs_path = get_pareidolia_bids(FOLDERPATH, subj, task, run, stage = 'epo_RT', cond=None)
                epochs = mne.read_epochs(epochs_path)
                #Si vous voulez comparer les epochs entières (8sec), il est mieux de laisser de côté le début et la fin des epochs.
                biotuner_metrics= compute_consonance(epochs, peaks_function='EMD', precision=1, tmin=-2, tmax=-0.1)
                #le nom du stage doit commencer par PSD, la fin du nom est à votre choix
                bt_file, bt_path = get_pareidolia_bids(FOLDERPATH, subj, task, run, stage = 'bt_RT_EMD_1hz_sub')
