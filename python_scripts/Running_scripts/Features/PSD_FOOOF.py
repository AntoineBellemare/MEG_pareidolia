...
def compute_psd(
    epochs, FREQ_BANDS, function=psd_array_welch, tmin=None, tmax=None, fmin=4, fmax=60
):
        ...
        for run in RUN_LIST[task]:
            try:
                epochs_name, epochs_path = get_pareidolia_bids(
                    FOLDERPATH, subj, task, run, stage="epo_RT_early3s_real", cond=None
                )
                ...
                psds_file, psds_path = get_pareidolia_bids(
                    ...
                    task,
                    run,
                    stage="PSD_RT_corrected_4-90Hz_nperseg300_earlyVSlate_3sec_real",
                )
