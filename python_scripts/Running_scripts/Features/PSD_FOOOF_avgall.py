...
def compute_bands(
    ...
    for t in range(len(epochs[:].get_data())):
        ...
        for elec in range(len(psds)):
            if epochs.metadata.iloc[t]["parei"] == 0:
                psds[elec] = psds[elec] - slope_cond1[elec]
            elif epochs.metadata.iloc[t]["parei"] == 1:
                psds[elec] = psds[elec] - slope_cond2[elec]
    ...
    for task in TASK_LIST:
        ...
        for run in RUN_LIST[task]:
            try:
                ...
                psds_cond1 = np.average(psds_cond1, axis=1)
                psds_cond2 = np.average(psds_cond2, axis=1)
                print("FREQS ALL SHAPE: ", freqs.shape)
                print("PSDS ALL SHAPE: ", psds_cond1.shape)
                print("PSDS ALL SHAPE: ", psds_cond2.shape)

                psds_cond1_run.append(psds_cond1)
                psds_cond2_run.append(psds_cond2)

            except (FileNotFoundError, numpy.AxisError):
                pass
        # combine all first dimensions of arrays for each run
        psds_cond1_final = np.concatenate(psds_cond1_run, axis=0)
        psds_cond2_final = np.concatenate(psds_cond2_run, axis=0)
        # average across trials
        psds_cond1 = np.average(psds_cond1_final, axis=0)
        psds_cond2 = np.average(psds_cond2_final, axis=0)
        print("PSDS SHAPE: ", psds_cond1.shape)
        print("PSDS SHAPE: ", psds_cond2.shape)
        slopes_cond1 = []
        slopes_cond2 = []
        for elec in range(len(psds_cond1)):
            ...
            slopes_cond1.append(slope_cond1)
            slopes_cond2.append(slope_cond2)
        all_slopes_cond1.append(slopes_cond1)
        all_slopes_cond2.append(slopes_cond2)
    ...
    for task in TASK_LIST:
        for run in RUN_LIST[task]:
            try:
                ...
                epochs_short = mne.read_epochs(epochs_path)
                slopes_cond1 = all_slopes_cond1[SUBJ_LIST.index(subj)]
                slopes_cond2 = all_slopes_cond2[SUBJ_LIST.index(subj)]
                PSDS = compute_bands(
                    epochs_short,
                    FREQ_BANDS4,
                    psd_array_welch,
                    tmin=-1.5,
                    tmax=-0.5,
                    slope_cond1=slopes_cond1,
                    slope_cond2=slopes_cond2,
                    fmin=2,
                    fmax=90,
                )
                ...
                psds_file, psds_path = get_pareidolia_bids(
                    ...
                    task,
                    run,
                    stage="PSD_RT_expe_slope_shortepochs_corrected_maxpeaks8_2-90Hz",
                )
