...
# Get the path two directories back
two_dirs_back = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../../Functions")
)

# Add this path to sys.path
sys.path.insert(1, two_dirs_back)

# Now import your function
from MEG_pareidolia_utils import get_pareidolia_bids
import mne
...
def connectivity(
    subj, cond_names=None, run_list=None, method="pli", names=None, FOLDERPATH=None
):
    all_epochs = []
    epochs_name, epochs_path = get_pareidolia_bids(
        FOLDERPATH, subj, "pareidolia", "1", stage="epo_RT_post", cond=None
    )
    ...
    for run in run_list:
        try:
            epochs_name, epochs_path = get_pareidolia_bids(
                FOLDERPATH, subj, "pareidolia", run, stage="epo_RT_post", cond=None
            )
    ...
    fmin, fmax = (4.0, 8.0, 12.0, 20.0, 30.0), (8.0, 12.0, 20, 30.0, 50.0)
