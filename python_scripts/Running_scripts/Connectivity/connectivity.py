...
def connectivity(subj, cond_names=None, run_list=None, method='pli', names=None, FOLDERPATH=None,
    ...
    all_epochs = []
    epochs_name, epochs_path = get_pareidolia_bids(FOLDERPATH, subj, 'pareidolia', '1', stage = 'epo_long', cond=None)
...
for subj in SUBJ_LIST:
    ...
    con = connectivity(subj=subj, cond_names=cond_names, run_list=RUN_LIST['pareidolia'], FOLDERPATH=FOLDERPATH,
                           names=names, method='wpli', tmin=0, tmax=8, )
    par.append(con[0].get_data(output='dense')[:, :, :])
    nopar.append(con[1].get_data(output='dense')[:, :, :])

np.save('connectivity_par_WPLI', par)
np.save('connectivity_nopar_WPLI', RT_nopar)
par_connect = sum(par)/len(par)
nopar_connect = sum(nopar)/len(nopar)
