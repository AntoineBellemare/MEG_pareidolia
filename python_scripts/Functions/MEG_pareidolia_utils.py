...
def split_by_2_conditions_meta (data, epochs, conditions, data_type='PSD', metadata=None):
...
def get_pareidolia_bids(FOLDERPATH, subj, task , run, stage, cond=None):
    ...
    if 'events' in stage:
        pareidolia_bidsname = 'sub-{}_ses-recording_task-{}_run-{}_{}{}'.format(subj, task, run, stage, extension)
    else:
        if cond == None: # build basename with or without cond
            pareidolia_bidsname = 'sub-{}_ses-recording_task-{}_run-{}_{}{}'.format(subj, task, run, stage, extension)
        else:
            pareidolia_bidsname = 'sub-{}_ses-recording_task-{}_run-{}_{}_{}{}'.format(subj, task, run, cond, stage, extension)

    pareidolia_bidspath = os.path.join(FOLDERPATH, 'sub-{}'.format(subj), 'ses-recording', 'meg', pareidolia_bidsname)
    return pareidolia_bidsname, pareidolia_bidspath


def reformat_events_meta (events, FDlist=None, RT_thresh = 5000, task = 'pareidolia', run = None,n_objects_list=None):
    ...
    if task == 'pareidolia':
        ...
        for e in range(len(events)):
            try:
                ...
                while events[e][2] == 6 and events[e+1][2] == 6: # remove events that correspond to pressing the response button more than one                                                               # time during a single trial
                    print('remove double press')
                    events = np.delete(events,(e+1), axis=0)
                if events[e][2] == 6 and events[e+2][2] == 6: # Remove button presses that happen after the end of the image
                    events = np.delete(events,(e+2), axis=0)
                if events[e][2] == 5 and events[e+1][2] == 6: # Remove button presses that happen after the end of the image
                    events = np.delete(events,(e+1), axis=0)
            except:
                pass
        ...
        for e in range(len(events)):
            try:
                if events[e][2] == 4:
                    m+=1
                    if events[e+1][2] == 6 and not n_objects_list[m] >= 1: #remove events when no response for number of object
                        print('remove event')
                        #events[e+1][2] = 666
                        events = np.delete(events,(e+1), axis=0)
            except IndexError:
        ...
        for e in range(len(events)):
            if events[e][2] == 4:
                ...
                try:
                    if events[e+1][2] != 6 and n_objects_list[m] >= 1: #identity trials when no motor response by number of objects is reported

                        events[e][2] = 44
                        events = np.insert(events, [e+1], [events[e][0]+100, 0, 666], axis=0)
                except:
                    pass
...
def earlyVSlate (events, RT_thresh):
    ...
    for e in range(len(events[:, 2])):
        ...
        if (events[e][2] == 44 and events[e+1][2] == 6 and ((events[e+2][0])-(events[e+1][0]))<=RT_thresh):
            ...
            i+=1
        #Early
        elif (events[e][2] == 44 and events[e+1][2] == 6 and ((events[e+2][0])-(events[e+1][0]))>RT_thresh):
...
def combine_metadata_PSD_complexity (subj_list, run_list, task, epo_stage=None, cp_stage=None, psd_stage=None, save=False,
    ...
    for s in subj_list:
        ...
        for run in run_list:
            try:
                ...
                if LZ is not None:
                    LZ_file, LZ_path = get_pareidolia_bids(FOLDERPATH, s, task, run, stage = 'array_comp_LZ_RS')
                ...
                if complex_imports is not None:
                    for names in complex_imports:
                        df_ = total_df
                        print(names[0])
                        total_df = import_complexity(names[0], names[1], df_, task=task, subj=s, run=run)
    ...
    if save is True:
        save_name = '../../Merged_dataframes/df_ALL_metadata_MEG_sub{}to{}_{}_{}.csv'.format(subj_list[0], subj_list[-1], epo_stage, savename)
...
def FOOOF_aperiodic(data, sf, precision=0.1, max_freq=80, noverlap=None,
...
def merge_multi_GLM(path, n_electrodes=270, graph=False, ch_xy=None, savename='_'):
    ...
    if graph is True:
        for e, effect in enumerate(list(dict_final.keys())):
            ...
            vmax = extreme
            vmin = -extreme
            reportpath = '../../OUTPUTS/GLM_topomaps/fig_GLM_'+savename+effect+'.png'
            print(reportpath)
