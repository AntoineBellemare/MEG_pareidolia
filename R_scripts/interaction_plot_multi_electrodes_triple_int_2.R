...
fixed_effect_name <- "gamma3.z:parei:DAT.z"  # Replace with your desired fixed effect name
...
regions_of_interest <- c("OL", "OR", "CL", "CR", "OZ", "CZ")  # Example region
  ...
  if (file.exists(full_path)) {
    ...
    if (fixed_effect_name %in% names(fixef(model))) {
      ...
      if ((effect_direction == "positive" && effect_value > 0 && p_value < 0.001) ||
        ...
        preds <- ggpredict(model, terms = c("DAT.z[all]", "gamma3.z[all]", "parei[all]"), ci.lvl=0.01)
